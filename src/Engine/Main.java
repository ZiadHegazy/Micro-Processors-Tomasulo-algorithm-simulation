...
public class Main {
	...
	public static ArrayList<Register> usedRegisters=new ArrayList<>();
...
//		start();
	...
	public static boolean isUsedRegister(String tag,ArrayList<Register> arr){
		for(int i=0;i<arr.size();i++){
			if(arr.get(i).getTag().equals(tag))
				return true;
		}
		return false;
	}
	...
	public void parseCode(ArrayList<String> arr){
		...
		for(int i=0;i<arr.size();i++){
			...
			for(int j=0;j<registerFile.length;j++){
				if(registerFile[j]!=null){
					String[] split=arr.get(i).split(" ")[1].split(",");
					if(split.length==2){
						if((registerFile[j].getTag().toLowerCase().equals(split[0].toLowerCase()) || 
								registerFile[j].getTag().toLowerCase().equals(split[1].toLowerCase()))&&
								!usedRegisters.contains(registerFile[j])){
							usedRegisters.add(registerFile[j]);
							System.out.println(registerFile[j].getTag());
						}
					}else if(split.length==3){
						if((registerFile[j].getTag().toLowerCase().equals(split[0].toLowerCase()) || 
								registerFile[j].getTag().toLowerCase().equals(split[1].toLowerCase())||
								registerFile[j].getTag().toLowerCase().equals(split[2].toLowerCase())) &&
								!usedRegisters.contains(registerFile[j])){
							usedRegisters.add(registerFile[j]);
							System.out.println(registerFile[j].getTag());
						}
	...
	public static ArrayList<String[]> myClone (ArrayList<Object> s){
		ArrayList<String[]> newArr = new ArrayList<String[]>();
		for (int i = 0; i < s.size(); i++) {
			if(s.get(i) instanceof ReservationStation){
				newArr.add(new String[] { ((ReservationStation)s.get(i)).getTime()+"" , ((ReservationStation)s.get(i)).getTag()});
			}else if(s.get(i) instanceof LoadStation){
				newArr.add(new String[] { ((LoadStation)s.get(i)).getTime()+"" , ((LoadStation)s.get(i)).getTag()});
				
			}
			else if(s.get(i) instanceof StoreStation){
				newArr.add(new String[] { ((StoreStation)s.get(i)).getTime()+"" , ((StoreStation)s.get(i)).getTag()});
	...
	public static ArrayList<String> myCloneString (ArrayList<String> s){
		ArrayList<String> newArr = new ArrayList<String>();
		for (int i = 0; i < s.size(); i++) {
			newArr.add(s.get(i));
		}
	...
	public static ArrayList<Register> myCloneReg (ArrayList<Register> s){
		ArrayList<Register> newArr = new ArrayList<Register>();
		for (int i = 0; i < s.size(); i++) {
			newArr.add(s.get(i));
		}
		return newArr;
...
//		divCylces=x;
		...
		while(!instructions.isEmpty() ||  !isEmptyStation(additionStation) 
			...
			ArrayList<String[]> arrExec=myClone(executingInstruction);
			System.out.println("Exce2: "+arrExec);
			...
			for(int i=0;i<usedRegisters.size();i++){
				int index=Integer.parseInt(usedRegisters.get(i).getTag().substring(1));
				usedRegisters.set(i,registerFile[index]);
			}
			ArrayList<String[]> arr2=new ArrayList<String[]>();
			for(int i=0;i<myCloneReg(usedRegisters).size();i++){
				arr2.add(new String[]{usedRegisters.get(i).getTag(),usedRegisters.get(i).getValue()+"",usedRegisters.get(i).getQi()});
			}
			System.out.println("Exce2: "+myCloneReg(usedRegisters));
			cyclesData.add(new DataCycle(cycle, multiplyStation.clone(), additionStation.clone(), loadBuffer.clone(), storeBuffer.clone(), arr2,arrExec , myCloneString(waitingInstructions) ));
			cycle++;
		}
